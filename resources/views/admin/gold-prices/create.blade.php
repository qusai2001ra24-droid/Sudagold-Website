...
@section('page_title', 'Add Gold Price')

@section('content')
<form method="POST" action="{{ route('admin.gold-prices.store') }}" class="space-y-6 max-w-2xl">
    @csrf
    <div class="bg-[#111111] border border-[#1f1f1f] rounded-xl p-6">
        <div class="space-y-4">
            ...
            <div>
                ...
                <input type="number" step="0.01" name="price_per_gram" value="{{ old('price_per_gram') }}" required
                    class="w-full px-4 py-2 bg-[#0a0a0a] border border-[#1f1f1f] rounded-lg text-white focus:outline-none focus:border-[#d4af37]">
            ...
            <div>
                ...
                <input type="number" step="0.01" name="price_per_ounce" value="{{ old('price_per_ounce') }}"
                    class="w-full px-4 py-2 bg-[#0a0a0a] border border-[#1f1f1f] rounded-lg text-white focus:outline-none focus:border-[#d4af37]">
            </div>
            <div>
            ...
            <div>
                <label class="flex items-center gap-3">
                    <input type="checkbox" name="set_current" value="1" {{ old('set_current') ? 'checked' : '' }}
